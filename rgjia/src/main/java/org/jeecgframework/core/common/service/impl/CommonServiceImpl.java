...
import com.yisi.framework.common.DataListParam;
...
public List<Map<String, Object>> findListForJdbc(String sql,DataListParam dataParam, Object... objs){
		
		if(StringUtils.isNotBlank(dataParam.getSort())){
			sql = sql + " order by " + dataParam.getSort() + " ";
			
			if(StringUtils.isNotBlank(dataParam.getOrder())){
				sql = sql + " " + dataParam.getOrder() + " ";
			}
		}
		
		
		if(null == dataParam.getPage()){
			dataParam.setPage(1);
		}
		if(null == dataParam.getRows()){
			dataParam.setRows(10);
		}
		
		
		return commonDao.findForJdbcParam(sql, dataParam.getPage(), dataParam.getRows(), objs);
	}
