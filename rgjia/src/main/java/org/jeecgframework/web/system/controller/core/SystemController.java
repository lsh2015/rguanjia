...
//			depart.setOrgCode(orgCode);
    ...
    public AjaxJson filedeal(HttpServletRequest request, HttpServletResponse response) {
        ...
        String ctxUrl=ResourceUtil.getConfigByName("webUploadurl");//demo中设置为D://upFiles,实际项目应因事制宜
        try {
	        //如果是上传操作
	        if("1".equals(upFlag)){
				...
				msg="上传成功";
				j.setMsg(msg);
				String dbpath=ctxUrl + File.separator + nowday+File.separator+fileName;
				j.setObj(dbpath);
