...
public class SystemServiceImpl extends CommonServiceImpl implements SystemService {
	...
	public TSType getType(String typecode, String typename, TSTypegroup tsTypegroup) {
		// TSType actType = commonDao.findUniqueByProperty(TSType.class,
		// "typecode", typecode,tsTypegroup.getId());
		List<TSType> ls = commonDao.findHql("from TSType where typecode = ? and typegroupid = ?", typecode,
				tsTypegroup.getId());
		TSType actType = null;
	...
	public String getValueByName(String paramName) {
		List<YsParamEntity> paramList = this.findByProperty(YsParamEntity.class, "paramName", paramName);

		String message = "";
		if (paramList.size() > 0) {
			YsParamEntity param = paramList.get(0);
			message = param.getParamValue();
		}
				...
				for (String MyoperationCode : operationCodes) {
					...
					if (operation.getOperationcode().startsWith(".") || operation.getOperationcode().startsWith("#")) {
						if (operation.getOperationType().intValue() == Globals.OPERATION_TYPE_HIDE) {
							...
							out.append("$(\"" + operation.getOperationcode().replaceAll(" ", "")
	...
	public void addDataLog(String tableName, String dataId, String dataContent) {
		...
		Integer integer = commonDao.singleResult("select max(versionNumber) from TSDatalogEntity where tableName = '"
				+ tableName + "' and dataId = '" + dataId + "'");
		if (integer != null) {
